...
async function loadHolidays(year){
  try{
    ...
    if(!res.ok) throw new Error('holidays.json load failed');
...
// 기준: 2025-08-07 = '오전1'  → 2025-08-08 은 '오전2'
const patternStartDate = new Date(2025,7,7); // (YYYY, M0, D)

function getShift(date){
  const ms = 24*60*60*1000;
  const diff = Math.floor((new Date(date.getFullYear(),date.getMonth(),date.getDate()) - patternStartDate)/ms);
